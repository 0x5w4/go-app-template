...
type SupportFeatureService interface {
	...
	ImportPreview(ctx context.Context, req *ImportPreviewSupportFeatureRequest) ([]*entity.SupportFeaturePreview, error)
...
func NewSupportFeatureService(config *config.Config, repo repo.Repository, logger logger.Logger, auth auth.AuthService, validate *validator.Validate) SupportFeatureService {
...
func (s *supportFeatureService) Create(ctx context.Context, req *CreateSupportFeatureRequest) (*entity.SupportFeature, error) {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "HELP_SERVICE.CREATE")
	if err != nil {
		return nil, err
	}
	...
	atomicOperation := func(txRepo mysql.MySQLRepository) error {
		for {
			sfCode, err := util.GenerateCode(constant.CodePefix["support_feature"], 5)
			if err != nil {
				return err
			}
...
func (s *supportFeatureService) BulkCreate(ctx context.Context, req *BulkCreateSupportFeatureRequest) ([]*entity.SupportFeature, error) {
	...
	atomicOperation := func(txRepo mysql.MySQLRepository) error {
		...
		for i := range req.SupportFeatures {
			...
			for range 100 {
				generatedCand, err := util.GenerateCode(constant.CodePefix["support_feature"], 5)
				if err != nil {
		...
		if len(itemsToRegenerateIndices) > 0 {
			for _, itemIndex := range itemsToRegenerateIndices {
				...
				for range 100 {
					candidateCode, err := util.GenerateCode(constant.CodePefix["support_feature"], 5)
					if err != nil {
						return exception.Wrap(err, exception.TypeInternalError, exception.CodeInternalError, "Failed to generate code")
					}
...
func (s *supportFeatureService) Find(ctx context.Context, req *FindSupportFeaturesRequest) ([]*entity.SupportFeature, int, error) {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "HELP_SERVICE.READ")
	if err != nil {
		return nil, 0, err
	}
...
func (s *supportFeatureService) Update(ctx context.Context, req *UpdateSupportFeatureRequest) (*entity.SupportFeature, error) {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "HELP_SERVICE.UPDATE")
	if err != nil {
		return nil, err
	}
...
func (s *supportFeatureService) Delete(ctx context.Context, req *DeleteSupportFeatureRequest) error {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "HELP_SERVICE.DELETE")
	if err != nil {
		return err
	}
...
func (s *supportFeatureService) ImportPreview(ctx context.Context, req *ImportPreviewSupportFeatureRequest) ([]*entity.SupportFeaturePreview, error) {
	if req.AuthParams == nil || req.AuthParams.Claims == nil {
		return nil, exception.New(exception.TypePermissionDenied, exception.CodeForbidden, "Token payload not provided")
	}

	ok, authErr := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "HELP_SERVICE.CREATE")
	if authErr != nil {
	...
	if len(rows) < 2 {
		return make([]*entity.SupportFeaturePreview, 0), nil
	}
	...
	previews := make([]*entity.SupportFeaturePreview, 0, len(rows)-1)
	keysToValidate := make([]string, 0, len(rows)-1)
	namesToValidate := make([]string, 0, len(rows)-1)

	for i, row := range rows[1:] {
		excelRowNumber := i + 2
		sf := &entity.SupportFeaturePreview{Row: excelRowNumber}
	...
	allProcessedSFs := make([]*entity.SupportFeaturePreview, 0, len(previews))
	errSFs := make([]*entity.SupportFeaturePreview, 0, len(previews))
