...
func NewUserService(config *config.Config, repo repo.Repository, logger logger.Logger, auth auth.AuthService) UserService {
	return &userService{
		config: config,
		repo:   repo,
		logger: logger,
...
func (s *userService) Create(ctx context.Context, req *CreateUserRequest) (*entity.User, error) {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "USER.CREATE")
	if err != nil {
		return nil, err
	}
...
func (s *userService) Update(ctx context.Context, req *UpdateUserRequest) (*entity.User, error) {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "USER.UPDATE")
	if err != nil {
		return nil, err
	}
	...
	if req.Update.Password != nil && *req.Update.Password != "" {
		hashedPassword, err := util.HashPassword(*req.Update.Password)
		if err != nil {
...
func (s *userService) Delete(ctx context.Context, req *DeleteUserRequest) error {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "USER.DELETE")
	if err != nil {
		return err
	}
	...
	if req.AuthParams.Claims.UserID == req.UserID {
...
func (s *userService) Find(ctx context.Context, req *FindUserRequest) ([]*entity.User, int, error) {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "USER.READ")
	if err != nil {
		return nil, 0, err
	}
