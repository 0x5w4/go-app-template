...
import (
	...
	"goapptemp/internal/domain/service/pubsub"
)
...
func NewClientService(config *config.Config, repo repo.Repository, logger logger.Logger, auth auth.AuthService, pubsub pubsub.PubsubService) ClientService {
...
func (s *clientService) Create(ctx context.Context, req *CreateClientRequest) (*entity.Client, error) {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "CLIENT.CREATE")
	if err != nil {
		return nil, err
	}
	...
	atomicOperation := func(txRepo mysql.MySQLRepository) error {
		for {
			clientCode, err := util.GenerateCode(constant.CodePefix["client"], 6)
			if err != nil {
		...
		if s.config.App.UsePubsub && isIconBase64 {
			...
			userLog := strconv.FormatUint(uint64(req.AuthParams.Claims.UserID), 10)
...
func (s *clientService) Find(ctx context.Context, req *FindClientsRequest) ([]*entity.Client, int, error) {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "CLIENT.READ")
	if err != nil {
		return nil, 0, err
	}
...
func (s *clientService) Update(ctx context.Context, req *UpdateClientRequest) (*entity.Client, error) {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "CLIENT.UPDATE")
	if err != nil {
		return nil, err
	}
...
func (s *clientService) Delete(ctx context.Context, req *DeleteClientRequest) error {
	...
	ok, err := s.auth.AuthorizationCheck(ctx, req.AuthParams.Claims.UserID, "CLIENT.DELETE")
	if err != nil {
		return err
	}
